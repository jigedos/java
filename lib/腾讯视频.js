...
var rule = {
        ...
        lazy: $js.toString(() => {
                try {
                        ...
                        if (bata.url.includes("http")) {
                                input = {
                                        header: { 'User-Agent': "" },
                                        parse: 0, url: bata.url, jx: 0,
                                        danmaku: "" + input.split("?")[0]
                                };
                        } else {
                                input = {
                                        header: { 'User-Agent': "" },
                                        parse: 0, url: input.split("?")[0], jx: 1,
                                        danmaku: "" + input.split("?")[0]
                                };
                        }
                } catch {
        ...
        一级: $js.toString(() => {
                ...
                if (fyclass === 'mini_series') {
                        ...
                        let requestBody = {
                                "page_params": {
                                        "page_type": "channel", "page_id": "120188", "scene": "channel",
                                        "new_mark_label_enabled": "1", "vl_to_mvl": "1",
                                        "free_watch_trans_info": "{\"ad_frequency_control_time_list\":{}}",
                                        "ad_exp_ids": "100000", "skip_privacy_types": "0", "support_click_scan": "1"
                                },
                                "page_bypass_params": {
                                        "params": {
                                                "platform_id": "2", "caller_id": "3000010", "data_mode": "default",
                                                "user_mode": "default", "page_type": "channel", "page_id": "120188",
                                                "scene": "channel", "new_mark_label_enabled": "1"
                                        },
                                        "scene": "channel", "app_version": ""
                                },
                        ...
                        try {
                                ...
                                if (json.ret === 0 && json.data && json.data.CardList) {
                                        if (json.data.has_next_page && json.data.page_context) {
                                                try {
                                                        storage0.setItem(cacheKey, JSON.stringify({ page: fypage, nextContext: json.data.page_context }));
                                                } catch (e) { }
                                        }
                                                ...
                                                if (card.type === '_eco_video_staggered' && card.children_list && card.children_list.card_list) {
                                                        ...
                                                        cards.forEach(function (item) {
                                                                if (item.type === '_eco_video_staggered_drama_item' && item.params) {
                                                                        let cid = item.params.cid || '';
                                                                        let posterInfo = {};
                                                                        let markInfo = {};
                                                                        try { posterInfo = JSON.parse(item.params.poster || '{}'); } catch (e) { }
                                                                        try { markInfo = JSON.parse(item.params.mark_label_list || '{}'); } catch (e) { }
                                                                        ...
                                                                        if (cid && title) {
                                                                                d.push({ title: title, img: img, desc: remarks, url: cid });
                                                                        }
                                                                }
                                                        });
                                                }
                        ...
                        let channel_id = classMap[fyclass];
                        if (channel_id) {
                                let apiUrl = 'https://pbaccess.video.qq.com/trpc.universal_backend_service.page_server_rpc.PageServer/GetPageData?video_appid=1000005&vplatform=2&vversion_name=8.9.10&new_mark_label_enabled=1';

                                let filterParts = [];
                                if (!fl.sort) fl.sort = "75";
                                for (let key in fl) {
                                        if (fl[key] !== '' && fl[key] !== '-1' && fl[key] !== '全部') {
                                ...
                                let pageContext = "";
                                let cacheKey = 'qq_ctx_' + fyclass + '_' + filterStr;
                                if (fypage > 1) {
                                        try {
                                                let cachedContext = storage0.getItem(cacheKey);
                                                if (cachedContext) {
                                ...
                                try {
                                        ...
                                        if (json.data) {
                                                if (json.data.has_next_page && json.data.next_page_context) {
                                                        try {
                                                                storage0.setItem(cacheKey, JSON.stringify({ page: fypage, nextContext: json.data.next_page_context }));
                                                        } catch (e) { }
                                                }

                                                let moduleList = json.data.module_list_datas || [];
                                                if (moduleList.length > 0) {
                                                        let lastModuleList = moduleList[moduleList.length - 1];
                                                        let moduleDatas = lastModuleList.module_datas || [];
                                                        if (moduleDatas.length > 0) {
                                                                let lastModuleData = moduleDatas[moduleDatas.length - 1];
                                                                let itemDatas = (lastModuleData.item_data_lists && lastModuleData.item_data_lists.item_datas) ? lastModuleData.item_data_lists.item_datas : [];

                                                                itemDatas.forEach(function (its) {
                                                                        if (its.item_params && its.item_params.cid) {
                                                                                let p = its.item_params;
                                                                                let cid = p.cid;
                                                                                let title = p.mz_title || p.title || '';

                                                                                let pic = p.new_pic_hz || p.new_pic_vt || p.pic || '';
                                                                                if (pic && pic.startsWith('//')) pic = 'https:' + pic;

                                                                                let tag = {};
                                                                                try { tag = JSON.parse(p.uni_imgtag || p.imgtag || '{}'); } catch (e) { }

                                                                                let remarks = '';
                                                                                if (tag.tag_4 && tag.tag_4.text) remarks = tag.tag_4.text;

                                                                                if (cid && title) {
                                                                                        d.push({ title: title, img: pic, desc: remarks, url: cid });
                                                                                }
                                                                        }
                                                                });
                                                        }
        ...
        搜索: $js.toString(() => {
                ...
                let rawKeyword = input.split("/")[3] || input;
                if (rawKeyword.includes('=')) { rawKeyword = rawKeyword.split('=').pop(); }
                let keyword = decodeURIComponent(rawKeyword).replace(/\[|\]|\(|\)|\{|\}/g, ' ').trim();
                ...
                function vodSearch(keyword, page = 0) {
                        return request('https://pbaccess.video.qq.com/trpc.videosearch.mobile_search.MultiTerminalSearch/MbSearch?vplatform=2', {
                                body: JSON.stringify({
                                        ...
                                        uuid: "B1E50847-D25F-4C4B-BBA0-36F0093487F6", retry: 0, query: keyword,
                                        pagenum: page, isPrefetch: true, pagesize: 30, queryFrom: 0,
                                ...
                                headers: {
                                        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
                                        'Content-Type': 'application/json', 'Origin': 'https://v.qq.com', 'Referer': 'https://v.qq.com/'
                                },
                                method: 'POST'
                        });
                }
                ...
                try {
                        ...
                        if (json.data && json.data.normalList) processItemList(json.data.normalList.itemList);
                        if (json.data && json.data.areaBoxList) {
                                json.data.areaBoxList.forEach(box => { processItemList(box.itemList); });
                        }
                } catch (e) {
                        log("搜索出错: " + e.message);
                }
